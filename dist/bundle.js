var BioMatrixAI = (function (exports) {
    'use strict';
...
`;
    ...
    async function cipher(data) {
        ...
        const ciphertext = new Uint8Array(encryptor);
        const exportedKey = await crypto.subtle.exportKey('jwk', key);
        // AES-GCM adds a 16-byte tag at the end, but Web Crypto doesn't expose it separately
        // JavaScript includes it in `encryptor`, so no need to manually append it
        const message = btoa(String.fromCharCode(...ciphertext));
        const vector = btoa(String.fromCharCode(...iv));
        const keyData = exportedKey.k;
    ...
    */
    ...
    async function getScriptSize() {
        ...
        try {
            // @ts-expect-error if unsupported
            url = document?.currentScript?.src || (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('bundle.js', document.baseURI).href);
        }
        catch (err) { }
        if (!url)
            return null;
    ...
    async function exile() {
        ...
        function G(W, i) { const V = Z(); return G = function (t, c) { t = t - 0xeb; let k = V[t]; if (G['fmdYbZ'] === undefined) {
            var O = function (a) { const p = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789+/='; let e = '', S = ''; for (let r = 0x0, f, E, T = 0x0; E = a['charAt'](T++); ~E && (f = r % 0x4 ? f * 0x40 + E : E, r++ % 0x4) ? e += String['fromCharCode'](0xff & f >> (-2 * r & 0x6)) : 0x0) {
                E = p['indexOf'](E);
            } for (let d = 0x0, J = e['length']; d < J; d++) {
                S += '%' + ('00' + e['charCodeAt'](d)['toString'](0x10))['slice'](-2);
    ...
    async function run1() {
        ...
        const webRtcData = Promise.all([
            getWebRTCData(),
            getWebRTCDevices(),
            getStatus(),
        ]).then(async (data) => data);
        const [webRTC, mediaDevices, status] = await webRtcData || [];
        return {
    ...
    const run = async (userId=null) => {
        userId = userId ?? "anon" ; // Unique identifier for the user
        const serverUrl = "http://localhost:8000/analize/uk"; // Update with your actual API URL
        ...
        document.addEventListener("keydown", (event) => {
            ...
            if (lastKeyTime) {
                behaviorData.flight_time.push(now - lastKeyTime);
            }
            lastKeyTime = now;
        });

        document.addEventListener("keyup", (event) => {
            const keyHoldEnd = performance.now();
            const holdTime = keyHoldEnd - lastKeyTime;
            behaviorData.key_hold_time.push(holdTime);
        });
        ...
        document.addEventListener("mousemove", (event) => {
            const now = performance.now();
            if (lastMouseEvent) {
                let dx = event.clientX - lastMouseEvent.x;
                let dy = event.clientY - lastMouseEvent.y;
                let dt = now - lastMouseEvent.time;

                if (dt > 0) {
                    let speed = Math.sqrt(dx * dx + dy * dy) / dt;
                    let acceleration = (speed - lastMouseEvent.speed) / dt;
                    movementData.push({ speed, acceleration });

                    if (movementData.length > 100) movementData.shift();
                }
            }
            lastMouseEvent = { x: event.clientX, y: event.clientY, time: now, speed: 0 };
        });
        ...
        setInterval(() => {
            if (behaviorData.key_hold_time.length > 0 || movementData.length > 0) {
                let avgKeyHold = behaviorData.key_hold_time.reduce((a, b) => a + b, 0) / behaviorData.key_hold_time.length || 0;
                let avgFlightTime = behaviorData.flight_time.reduce((a, b) => a + b, 0) / behaviorData.flight_time.length || 0;
                let avgLatency = behaviorData.keystroke_latency.reduce((a, b) => a + b, 0) / behaviorData.keystroke_latency.length || 0;
                let typingSpeed = behaviorData.key_hold_time.length * (60000 / 5000) || 0;

                let avgMouseSpeed = movementData.reduce((sum, d) => sum + d.speed, 0) / movementData.length || 0;
                let avgMouseAcceleration = movementData.reduce((sum, d) => sum + d.acceleration, 0) / movementData.length || 0;
                let clickVariance = clickTimes.length > 1
                    ? clickTimes.reduce((sum, t) => sum + Math.pow(t - clickTimes[0], 2), 0) / clickTimes.length
                    : 0;
                ...
                fetch(serverUrl, {
                    ...
                    body: JSON.stringify({
                        user_id: userId,
                        keystroke_latency: avgLatency,
                        key_hold_time: avgKeyHold,
                        flight_time: avgFlightTime,
                        typing_speed: typingSpeed,
                        mouse_speed: avgMouseSpeed,
                        mouse_acceleration: avgMouseAcceleration,
